...
def run_linux_command(command: str, device_name: str = "device1"):
...
def run_command_tool(input_text: str) -> dict:
    ...
    Input format: "<device_name>: <command>" or just "<command>" (defaults to device1)
    Example: "device1: ifconfig -a" or "ifconfig -a"
    """
    try:
...
tools = [
    {
        ...
        "function": {
            ...
            "description": "Execute a command on device1. If device name is not specified, defaults to device1.",
            "parameters": {
                "type": "object",
                "properties": {
                    "input_text": {
...
- Your default device is device1 if not specified
